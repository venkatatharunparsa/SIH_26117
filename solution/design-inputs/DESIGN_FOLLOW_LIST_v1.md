...
This file is what we follow. Agenda, red-team, and PS completeness are the backing notes. **Folder:** `solution/`.

| File | Role |
|---|---|
| **This file** | Queue + why + significance + source of truth |
| `SKILL_LIBRARY_INTENT.md` | Skill library on tools/agents — catalog + task-scoped load |
| `DESIGN_REPO_MAP_v1.md` | Adopt / add / refuse vs existing repos (not a fork lock) |
...
| `DESIGN_PS_COMPLETENESS_v1.md` | Every PS clause mapped |
| `DESIGN_INVENTORY_v1.md` | Have vs hypothesis |
| `../sih_research/sovereign-ai-workbench-kb/01-problem-context/01-problem-statement.md` | **Official** PS text |
...
**Job.** Official local tools. System policy vs skills. **One orchestrator**, **separate agents** for defined tasks. File **write** is a gated tool. **Skill library** binds the matching `SKILL.md` (and its `allowed-tools`) to the agent for that task — not the whole library at once.

**Why required.** Description: act like an agent; plan; call local tools (file R/W, sandbox, spreadsheet, internal search); **iterate**. Background: Claude/Codex-class work. **User:** skill library on tools and agents for efficient work.

**Significance.** Swarm with shared full access = standing privilege. Specialists inherit **task grants**. Progressive disclosure keeps small models usable.
...
1. **WP-00 frozen** → `WP-00_FREEZE.md`.  
2. Walk one WP at a time (freeze must / may / never). **Current:** **WP-09** architect open (not frozen) → then **WP-10**.  
3. Later WPs may use frozen contracts only; may not reopen WP-00 without explicit decision.  
