...
| Skill library | On-disk **catalog** of `SKILL.md` packs. Orchestrator/agents/**tools** load the matching skill for efficient work | Glue every skill to every tool at once; Skills Hub / WAN install; weight training |
...
| 2 | Tools, system prompt, agents | **WP-07** + **skill library** | PS local tools. Specialists. Skills bind tools by task (see `SKILL_LIBRARY_INTENT.md`). |
...
WP-00 must state **plugin host + local-only install**, **models as pluggable catalog entities** (add/remove without workbench redesign), and a **skill library** (task-scoped load — see `SKILL_LIBRARY_INTENT.md`), or WP-08/WP-24/WP-07 will smuggle a WAN marketplace, hardcoded weights, and a skill dump.
